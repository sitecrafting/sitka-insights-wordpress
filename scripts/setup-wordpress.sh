...
main() {
  ...
  if ! [[ -f "$WP_DIR"/wp-content/plugins/sitka-insights-wordpress ]]
  then
    echo 'Linking Sitka Insights plugin directory...'
    ln -s "../../../" "$WP_DIR"/wp-content/plugins/sitka-insights-wordpress
  fi
...
EOF
  ...
  then
    ...
    then
      ...
      read -p "${BOLD}Site URL${NORMAL} (https://sitka-insights.lndo.site/): " URL
      URL=${URL:-'https://sitka-insights.lndo.site/'}

      read -p "${BOLD}Site Title${NORMAL} (Sitka Insights): " TITLE
      TITLE=${TITLE:-'Sitka Insights'}
      ...
      read -p "${BOLD}Admin password${NORMAL} (edi): " ADMIN_PASSWORD
      ADMIN_PASSWORD=${ADMIN_PASSWORD:-'edi'}
  ...
  wp --quiet plugin activate sitka-insights-wordpress classic-editor
  
  wp --quiet theme activate twentytwentyfive

  # uninstall stock themes
  wp theme uninstall twentytwentyfour twentytwentythree
